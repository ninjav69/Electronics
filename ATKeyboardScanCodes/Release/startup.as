	...
	; Compiler options:
	;
	; --chip=16F84A --ide=hitide --opt=none --summary=mem,file -G -M \
	; --asmlist --warn=0 --char=unsigned --double=24 --opt=+9 --opt=+asm \
	; --output=+intel --output=+mcof --runtime=+init --runtime=+osccal \
	; --runtime=+clear --runtime=+clib --runtime=+keep -oh main.p1 isr.p1 \
	; init.p1
	;
...
;	5 bytes of RAM objects in bank 0 to zero

	psect	init
	global	__Lrbss_0
	movlw	low(__Lrbss_0)		;load the low address into FSR
	movwf	FSR
	movlw	low(__Lrbss_0+5)		;load the high address+1 into W

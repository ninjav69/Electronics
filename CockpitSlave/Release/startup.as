	...
	; Compiler options:
	;
	; --chip=16F877A --ide=hitide --opt=none --summary=mem,file -G -M \
	; --asmlist --warn=0 --char=unsigned --double=24 --opt=+9 --opt=+asm \
	; --output=+intel --output=+mcof --runtime=+init --runtime=+osccal \
	; --runtime=+clear --runtime=+clib --runtime=+keep -oCockpitSlave.hex \
	; sb.p1 rof.p1 main.p1 isr.p1 init.p1 fifo.p1
	;
...
;	48 bytes of RAM objects in bank 1 to zero

	psect	init
	global	__Lrbss_1
	movlw	low(__Lrbss_1)		;load the low address into FSR
	movwf	FSR
	movlw	low(__Lrbss_1+48)		;load the high address+1 into W
...
;	48 bytes of RAM objects in bank 2 to zero
	...
	movwf	FSR
	movlw	low(__Lrbss_2+48)		;load the high address+1 into W
	fcall	clear_ram		;go and zero that range of addresses

;	No RAM objects to clear in bank 3
;	12 bytes of RAM objects in common bank to zero

	psect	init
	global	__Lcommon
	movlw	low(__Lcommon)		;load the low address into FSR
	movwf	FSR
	movlw	low(__Lcommon+12)		;load the high address+1 into W
	fcall	clear_ram		;go and zero that range of addresses
